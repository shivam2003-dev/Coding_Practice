...
public:
    ...
    void push(int x) {
        ...
        q.push(x);            //   simply push the element
    }
    
    int pop() {
        
        while(q.size()!=1){       // push all other elements in the other queue except the last one
            p.push(q.front());
            q.pop();
        }
    ...
    int top() {
        ...
        int k = pop();          // top can be seen as just a pop function followed by a push 
